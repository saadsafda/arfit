...
export class DeleteUserBodyImageUseCase {
  ...
  async execute(userIdentifier, userType) {
    ...
    repoReturnedValues = await this.userBodyImageRepository.deleteUserBodyImage(
      existingUser.getID()
    );
    ...
    switch (userType) {
      case CommonConstants.REGISTERED_USER_TYPE:
        repoReturnedValues =
          await this.userRepository.markUserBodyUnScannedByEmail(
            userIdentifier
          );
        break;
      case CommonConstants.GUEST_USER_TYPE:
        repoReturnedValues =
          await this.userRepository.markUserBodyUnScannedByID(userIdentifier);
        break;
      default:
        return new APIResponse(
          new APIResponseMessage(
    ...
    return new APIResponse(
      new APIResponseMessage(
        APIResponseConstants.USER_BODY_IMAGE_DELETED_MESSAGE,
        APIResponseConstants.USER_BODY_IMAGE_DELETED_MESSADE_CODE
      ),
