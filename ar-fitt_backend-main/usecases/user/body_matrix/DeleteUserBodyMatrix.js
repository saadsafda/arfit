...
export class DeleteUserBodyMatrixUseCase {
  ...
  async execute(userEmail) {
    ...
    repoReturnedValues =
      await this.userBodyMatrixRepository.deleteUserBodyMatrix(
        existingUser.getID()
      );
    ...
    repoReturnedValues = await this.userRepository.markUserBodyUnScannedByEmail(
      userEmail
    );
    if (repoReturnedValues != null) {
      return new APIResponse(
    ...
    return new APIResponse(
      new APIResponseMessage(
        APIResponseConstants.USER_BODY_MATRIX_DELETED_MESSAGE,
        APIResponseConstants.USER_BODY_MATRIX_DELETED_MESSADE_CODE
      ),
